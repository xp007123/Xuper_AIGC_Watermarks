...
func (t *Chain) SubmitTx(ctx xctx.XContext, tx *lpb.Transaction) error {
	...
	reqInvoke := tx.GetContractRequests()
	for _, req := range reqInvoke {
		args := req.GetArgs()
		inputBytes, ok := args["input"]
		if !ok {
			t.log.Warn("input key not found in args")
			continue
		}
		...
		if err != nil {
			t.log.Error("SubmitTx error", "error", err, "message", "error unmarshaling input data")
		}

		nonceValue, ok := inputData["nonce"]
		if !ok {
			t.log.Warn("nonce key not found in input data")
			continue
		}

		nonceStr, ok := nonceValue.(string)
		if !ok {
			t.log.Error("SubmitTx error", "error", "nonce value is not a string")
		}
		//替换nonce
		tx.Nonce = nonceStr
	}
