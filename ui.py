...
# amend
def show_copyright_message():
    text = "全部作品已链上存证,     txid ： " + str(end_process())
    return text
# amend
def end_process():
    # 发送"流程结束"请求
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        copyright_output = gr.Textbox(label="")

        copyright_btn = gr.Button(value="Copyright Protection", elem_id="copyright_btn")
        copyright_btn.click(fn=show_copyright_message, inputs=None, outputs=copyright_output)
