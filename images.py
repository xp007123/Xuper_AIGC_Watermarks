...
#amend
def split_path(path):
    # Use regular expression to split the string by either slash or backslash
    parts = re.split(r'[\\/]', path)
    return parts
#amend
def rebuild_name_with_prefix(parts):
    # Append the prefix to the last element of the parts list
    parts[-1] = parts[-1].replace("tmp","png")
    # Rebuild the path using the correct separator for the current operating system
    new_path = os.path.join(*parts)
    return new_path

#amend
def rebuild_path_with_prefix(parts, prefix="watermarks-"):
    # Append the prefix to the last element of the parts list
    parts[-1] = prefix + parts[-1].replace("tmp","png")
    # Rebuild the path using the correct separator for the current operating system
    new_path = os.path.join(*parts)
    return new_path
...
def save_image_with_geninfo(image, geninfo, filename, extension=None, existing_pnginfo=None, pnginfo_section_name='parameters'):
    ...
    """
    with open("D:\\code\\test.txt", "w", encoding='utf-8') as f:
        f.write(str(geninfo))
...
def save_image(image, path, basename, seed=None, prompt=None, extension='png', info=None, short_filename=False, no_prompt=False, grid=False, pnginfo_section_name='parameters', p=None, existing_info=None, forced_filename=None, suffix="", save_to_dirs=None):
    ...
    #amend
    update_nonce_if_empty()
    
    data = {
    'name': '图片名称',
    'infor': info,
    'prompt': 'prompt:  ' + str(prompt),
    'time': str(int(time.time()))
    }
    json_data = json.dumps(data)
    #发送到服务端
    update_process(json_data)

    # parts_replaced = split_path(filename)
    #保存水印图片   
    bwm1 = WaterMark(password_img=1, password_wm=1)
    #end_name = rebuild_name_with_prefix(parts_replaced)
    bwm1.read_img(str(fullfn_without_extension)+".png")
    wm = NONCE
    bwm1.read_wm(wm, mode='str')
    #new_path = rebuild_path_with_prefix(parts_replaced)
    bwm1.embed(str(fullfn_without_extension)+"_3we.png")
    len_wm = len(bwm1.wm_bit)
    print('Put down the length of wm_bit {len_wm}'.format(len_wm=len_wm))

    return fullfn, txt_fullfn

def update_nonce_if_empty():
    global NONCE
    if NONCE == "":
        NONCE = start_process()
...
def start_process():
    # 发送"流程开始"请求
    response = requests.post('http://localhost:8080/start')
...
def update_process(json_data):
    data = {'sdInfor': json_data}
    response = requests.post('http://localhost:8080/update', data=data)
    print(response.json()['message'])

#amend
def end_process():
    # 发送"流程结束"请求
    response = requests.post('http://localhost:8080/end')
    print(response.json()['message'])
